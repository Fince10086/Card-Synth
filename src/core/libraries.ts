...
export const EFFECT_LIBRARY: Record<string, ModuleDefinition> = {
  ...
  BitCrusher: {
    ...
    controls: [
      { path: "options.bits", kind: "range", label: "Bits", min: 1, max: 8, step: 1, formatter: (value: number) => `${Math.round(value)}${t(" bit")}` },
      { path: "options.wet", kind: "range", label: "Wet", min: 0, max: 1, step: 0.01, formatter: formatPercent },
    ],
  },
  Chebyshev: {
  ...
  Chorus: {
    ...
    controls: [
      { path: "options.frequency", kind: "range", label: "Rate", min: 0.1, max: 12, step: 0.1, formatter: formatHertz },
      { path: "options.delayTime", kind: "range", label: "Delay", min: 0.5, max: 10, step: 0.1, formatter: (value: number) => `${Math.round(value)} ${t("ms")}` },
      { path: "options.depth", kind: "range", label: "Depth", min: 0, max: 1, step: 0.01, formatter: formatPercent },
      { path: "options.type", kind: "select", label: "Type", options: SHARED_WAVE_OPTIONS },
  ...
  PitchShift: {
    ...
    controls: [
      { path: "options.pitch", kind: "range", label: "Pitch", min: -24, max: 24, step: 1, formatter: (value: number) => `${value > 0 ? "+" : ""}${value}${t(" st")}` },
  ...
  Tremolo: {
    ...
    controls: [
      ...
      { path: "options.depth", kind: "range", label: "Depth", min: 0, max: 1, step: 0.01, formatter: formatPercent },
      { path: "options.spread", kind: "range", label: "Spread", min: 0, max: 180, step: 1, formatter: (value: number) => `${Math.round(value)}${t("°")}` },
      { path: "options.wet", kind: "range", label: "Wet", min: 0, max: 1, step: 0.01, formatter: formatPercent },
    ],
  },
  Vibrato: {
