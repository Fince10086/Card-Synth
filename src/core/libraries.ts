...
export const SOURCE_LIBRARY: Record<string, ModuleDefinition> = {
  ...
  Oscillator: {
    ...
    controls: [
      ...
      { path: "options.frequency", kind: "range", label: "Frequency", min: 0.1, max: 100, step: 0.01, formatter: formatHertz, conditional: (module) => !(module.options as unknown as Record<string, unknown>)?.midiOn },
...
export const INPUT_LIBRARY: Record<string, ModuleDefinition> = {
  Pitch: {
    ...
    controls: [
      ...
      { path: "options.transpose", kind: "range", label: "Transpose", min: -12, max: 12, step: 1, conditional: (m) => (m.options as unknown as Record<string, unknown>)?.mode === "midi", formatter: (value: number) => `${value > 0 ? "+" : ""}${value} st` },
      { path: "options.octave", kind: "range", label: "Octave", min: -4, max: 4, step: 1, conditional: (m) => (m.options as unknown as Record<string, unknown>)?.mode === "midi", formatter: (value: number) => `Oct ${value > 0 ? "+" : ""}${value}` },
